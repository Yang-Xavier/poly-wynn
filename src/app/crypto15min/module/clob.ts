...
class Clob {
  ...
  public async init(): Promise<ClobClient> {
    ...
    this.clobClient = new ClobClient(host, 137, signer, creds, signatureType, funderAddress);
   ...
   */
  public async getOrder({ orderId }: { orderId: string }): Promise<PolymarketOrderResult | null> {
    if (!this.inited) {
      throw new Error("ClobClient not initialized. Please call init() first.");
    }
    let resp;
  ...
  public getBestPrices(
    ...
    const bestBid =
      orderBook.bids.length > 0 ? orderBook.bids[orderBook.bids.length - 1]?.price : null;
    const bestAsk =
      orderBook.asks.length > 0 ? orderBook.asks[orderBook.asks.length - 1]?.price : null;
