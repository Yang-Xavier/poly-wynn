...
export class Logger {
   ...
   */
  trade(
    type: "buy" | "sell" | "redeem" | "lost" | "balance" | "won" | "skip",
    orderResult: PolymarketOrderResult
  ): void {
    ...
    if (type === "redeem") {
      this.log(
        LogLevel.INFO,
        "trade",
        `${label[type]}[${type}], outcome: ${outcome}, amount: ${size_matched}`
      );
      return;
      ...
      this.log(
        LogLevel.INFO,
        "trade",
        `${label[type]}[${type}], no chance, skipped =====================================>`
      );
    } else {
      this.log(
        LogLevel.INFO,
        "trade",
        `${label[type]}[${type}], outcome: ${outcome}, totalPriceAmount: ${totalPriceAmount}, avgPrice: ${price}, rawOrderResult: ${JSON.stringify(orderResult)}`
      );
    }
  }
...
export const logData = (message: string, data?: any) =>
  getLoggerModule().customLog("data", LogLevel.INFO, message, data, false);
