...
export const calcBreakoutProbability = (
  ...
  const meanLogReturnPerStep = logReturns.reduce((sum, r) => sum + r, 0) / m;
  ...
  const meanLogST = Math.log(lastPrice) + meanLogReturnPerStep * stepsAhead;
  ...
  if (!isFinite(stdLogST) || stdLogST === 0) {
    const deterministic = lastPrice >= breakPrice ? { up: 1, down: 0 } : { up: 0, down: 1 };
    return {
...
export const calcDiffEnough = (
  ...
  const timeBasedRatio = calcAttenuationFactor(
    [diffFactor.reverse(), [0, startBefore]],
    distance,
    attenuationFactor[0],
    attenuationFactor[1]
  );
...
export const analyzeLastPeriodDecision = (
  ...
  thresholds: LastPeriodDecisionThresholds = {
    minEdge: 0.1,
    minProbability: 0.9,
    minProbabilityGap: 0.6,
    strongProbabilityGap: 0.8,
  }
  ...
  const breakout = calcBreakoutProbabilityFromTicks(ticks, timeToExpiryMs, priceToBeat);
  ...
  const fairUpProbability = upPayout > 1 ? 1 / upPayout : 0.5; // 赔率无效时退化为 0.5
  const fairDownProbability = downPayout > 1 ? 1 / downPayout : 0.5;
  ...
  const strongProbabilityGap = thresholds.strongProbabilityGap ?? 0.15; // 差 15% 以上视为强烈信号

  const trendThreshold = globalConfig?.stratgegy?.confidenceThreshold ?? 0.2;
  ...
  // 5.1 概率更大的一方
  const probDirection =
    upBreakProbability >= downBreakProbability ? OUTCOMES_ENUM.Up : OUTCOMES_ENUM.Down;
  const probValue = probDirection === OUTCOMES_ENUM.Up ? upBreakProbability : downBreakProbability;
  const otherProbValue =
    probDirection === OUTCOMES_ENUM.Up ? downBreakProbability : upBreakProbability;
  ...
  const edgeValue = probDirection === OUTCOMES_ENUM.Up ? edgeUp : edgeDown;
  const meetsEdge = edgeValue >= minEdge;

  const trendAligns = trend.direction === probDirection && trend.confidence >= trendThreshold;

  if (meetsProbLevel && meetsGap && meetsEdge) {
    if (trendAligns || probGap >= strongProbabilityGap) {
      recommendation = probDirection;
      reason = trendAligns ? "probability_and_trend_align" : "probability_strong_edge";
    } else {
...
export const analyzePositionExitDecision = (
  ...
  const currentOutcome = lastPrice >= priceToBeat ? OUTCOMES_ENUM.Up : OUTCOMES_ENUM.Down;
  ...
  if (currentOutcome === positionOutcome) {
    ...
    const winningProbability =
      positionOutcome === OUTCOMES_ENUM.Up ? upWinProbability : downWinProbability;
  ...
  const defaultHoldThreshold = globalConfig?.stratgegy?.breakProbabilityThreshold ?? 0.9;
  const holdProbabilityThreshold = thresholds.holdProbabilityThreshold ?? defaultHoldThreshold;
