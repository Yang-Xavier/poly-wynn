...
export class TailSweepStrategy {
  ...
  calculateMomentumFactor(): MomentumFactor {
    ...
    // 短期动量（最近几个点）
    const shortMomentum =
      (lastPrice - recentPrices[recentPrices.length - 3].value) /
  ...
  calculateTailProbability(): TailProbabilityResult {
    ...
    // 时间衰减调整
    // 剩余时间越少，当前价格的决定性越强
    const timeAdjustedProb =
      baseProbUp * timeDecay + (currentPrice > this.targetPrice ? 1 : 0) * (1 - timeDecay) * 0.5;
  ...
  generateSweepDecision(): TailDecision {
    const currentPrice = this.getCurrentPrice();
    const priceDiffPercent = (((currentPrice - this.targetPrice) / this.targetPrice) * 100).toFixed(
      2
    );
    ...
    if (valueEdgeUp > MIN_EDGE) {
      const kellyAmount = this.tailKellyCriterion(
        market.upOdds,
        tailProb.probUp,
        this.timeRemaining
      );
