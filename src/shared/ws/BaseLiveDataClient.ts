...
export abstract class BaseLiveDataClient {
  ...
  public readonly logger: IWsLogger;
  ...
  // onMessage 防抖定时器
  private messageDebounceTimer: NodeJS.Timeout | null = null;

  // onMessage 防抖参数缓存：存储最新的消息数据
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        this.ws.on("open", () => {
          this.logger.logInfo(`[${this.name}] WebSocket 连接已建立`);
          ...
          try {
            ...
            this.logger.logInfo(`[${this.name}] onOpen 处理异常: ${err}`);
          }
        ...
        this.ws.on("message", (data: WebSocket.Data) => {
          ...
          this.messageDebounceTimer = setTimeout(() => {
            if (this.latestMessageData !== null) {
        ...
        this.ws.on("error", (error: Error) => {
          this.logger.logInfo(`[${this.name}] WebSocket 错误: ${error}`);
        ...
        this.ws.on("close", (code: number, reason: Buffer) => {
          this.logger.logInfo(
            `[${this.name}] WebSocket 连接已关闭: ${code} - ${reason.toString()}`
          );
          this.isConnected = false;
          this.ws = null;
          ...
          if (!this.isManualDisconnect) {
            ...
            this.logger.logInfo(`[${this.name}] 主动断开连接，不进行重连`);
          }
        });
      } catch (error) {
        this.logger.logInfo(`[${this.name}] 连接失败: ${error}`);
        reject(error);
      }
    });
  }
  ...
  protected attemptReconnect(): void {
    if (this.isManualDisconnect) {
      this.logger.logInfo(`[${this.name}] 已主动断开，不再尝试重连`);
      return;
    }

    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      this.logger.logInfo(`[${this.name}] 达到最大重连次数，停止重连`);
      return;
    }
    ...
    setTimeout(() => {
      // 在真正重连前再次检查是否已经被主动断开
      if (this.isManualDisconnect) {
        this.logger.logInfo(`[${this.name}] 已主动断开，取消本次重连`);
        return;
      }
      this.connect().catch((error) => {
        this.logger.logInfo(`[${this.name}] 重连失败: ${error}`);
      });
    }, this.reconnectDelay);
  }
  ...
  disconnect(): void {
    ...
    if (!this.isConnected && !this.ws) {
      ...
      this.logger.logInfo(`[${this.name}] WebSocket 已是断开状态（主动关闭）`);
      return;
    }
    ...
    this.logger.logInfo(`[${this.name}] WebSocket 连接已断开`);
  }
}
