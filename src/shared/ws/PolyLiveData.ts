 ...
 */
export class PolyLiveDataClient extends BaseLiveDataClient {
  ...
  protected onMessage(data: WebSocket.Data): void {
    try {
      ...
      this.logger.logInfo(`[PolyLiveData] 解析消息失败: ${error} data: ${data.toString()}`);
    }
  }
  ...
  private sendSubscriptions(subscriptions: Subscription[]): void {
    ...
    this.logger.logInfo("[PolyLiveData] 发送订阅请求", message);
  }
  ...
  onWatchPriceChange(
    callback: (
      ...
      historyPriceList: { value: number; timestamp: number }[]
    ) => any
  ) {
