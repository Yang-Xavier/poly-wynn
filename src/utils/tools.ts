...
export const getMarketSlug15Min = (market: string = 'btc', intervalTimestamp: number) => {
...
export const isGreaterThan95 = (price: string) => {
    return parseFloat(price) >= 0.95;
}
...
export const isDiffBeatPrice = (price: string, priceToBeat: number, diffBeatPriceFactor: number) => {
    return Math.abs(Number(price) - Number(priceToBeat)) / Number(priceToBeat) >= diffBeatPriceFactor;
}